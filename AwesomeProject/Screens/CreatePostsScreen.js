...
import { 
 ...
 export const CreatePostsScreen = () => {
  ...
  const uploadPostToServer = async () => {
        ...
        <View style={styles.container}>
          ...
          keyboardVerticalOffset={Platform.OS === 'ios' ? -103 : -103}>
           ...
           <Camera style={styles.camera} ref={setCamera} >
            ...
            <Image style={{ width: 347, height: 244, borderRadius: 8,}} source={{uri: image}}/>
                  </View>}
            <TouchableOpacity
        activeOpacity={0.7}
        style={{...styles.snapContainer, backgroundColor: image? "#FFFfff4C" : "#FFF" }}
        onPress={takePhoto}
        >
        <MaterialCommunityIcons name="camera-enhance-outline" size={24} color={image? "#FFF" : "#BDBDBD"} />
    </TouchableOpacity>
    </Camera>
    </View>
    {image? <Text style={styles.text}>Редагувати фото</Text> : <Text style={styles.text}>Завантажте фото</Text>}
    ...
    <TouchableOpacity
        activeOpacity={0.7}
        style={{...styles.button, backgroundColor: image? "#FF6C00" : "#F6F6F6"}}
        onPress={onSubmit}
        >
      <Text style={{...styles.appButtonText, color: image? '#FFF' : '#BDBDBD',}}>
      Опубліковати
    </Text>
    </TouchableOpacity>
    <TouchableOpacity
